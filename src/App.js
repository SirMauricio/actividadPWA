...
function App() {
  ...
  useEffect(() => {
    ...
    // Ocultar splash después de 2 segundos si no se oculta desde Splash
    const splashTimer = setTimeout(() => setCargando(false), 2000);
  ...
  const agregarTarea = () => {
    ...
    setNuevaTarea("");
    mostrarNotificacion("Nueva tarea agregada", `"${nueva.texto}" se agregó a tu lista.`);
  };
  ...
  const eliminarTarea = (index) => {
    ...
    setTareas(nuevas);
    mostrarNotificacion("Tarea eliminada", `"${tareaEliminada.texto}" se eliminó de tu lista.`);
  };
  ...
  if (cargando) return <Splash onFinish={() => setCargando(false)} />;
        ...
        {tareas.map((tarea, index) => (
          <li key={index} className={tarea.completada ? "tarea completada" : "tarea"}>
            <span onClick={() => toggleCompletada(index)}>
              {tarea.texto} {tarea.completada && <FontAwesomeIcon icon={faCheck} />}
            </span>
